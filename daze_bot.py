        ...
        if name.strip():
            ...
            if i < len(att_raw):
                try:
                    ...
                    att = 0
            # แนบ code จาก members sheet ตาม index เดียวกัน
            # (ทั้งสอง sheet ลำดับสมาชิกเหมือนกัน จึง join by position ได้)
            code = codes_m[i].strip() if i < len(codes_m) else ""
            activity.append({"name": name.strip(), "code": code, "attendance": att})
...
def find_member(name: str):
    """ค้นหาจากรหัส (dz001) หรือ ชื่อในเกม แบบ case-insensitive + partial match"""
    n = name.strip().lower()
    members  = get_members()
    activity = get_activity()

    m = next((x for x in members
              if n == x["code"].lower() or n in x["name"].lower()), None)

    if not m:
        # ไม่เจอ member — ลองค้นใน activity ตรงๆ
        a = next((x for x in activity if n in x["name"].lower()), None)
        return None, a

    # ค้นชาจ activity ด้วย code ก่อน (กัน duplicate ชื่อในเกม)
    code = m.get("code", "").lower()
    if code:
        a = next((x for x in activity if x.get("code", "").lower() == code), None)
        if a:
            return m, a

    # Fallback: exact name match
    search_name = m["name"].lower()
    a = next((x for x in activity if search_name == x["name"].lower()), None)
    return m, a
...
async def help_cmd(ctx):
    ...
    embed.add_field(
        name="`!rank`",
        value="🏆 อันดับ **1–25** ตามคะแนนรวม",
        inline=False
    )
    embed.add_field(
        name="`!rank2`",
        value="🏆 อันดับ **26–50** ตามคะแนนรวม",
        inline=False
    )
    embed.add_field(
        name="`!กิจ [ชื่อ]`",
...
# ─────────────────────────────────────────
#  rank helper (shared logic)
# ─────────────────────────────────────────
async def send_rank_slice(ctx, start: int, end: int):
    """ส่ง ranking ช่วง start–end (1-based index)"""
    members  = get_members()
    sorted_m = sorted(members, key=lambda x: x["score"], reverse=True)
    medals   = ["🥇", "🥈", "🥉"]

    slice_m = sorted_m[start - 1 : end]   # slice ตาม index
    if not slice_m:
    ...
    embed = discord.Embed(
        title=f"🏆 Ranking อันดับ {start}–{actual_end} — DAZE",
        description="\n".join(lines),
        color=0xFEE75C
    )
    embed.set_footer(text=updated_footer())
