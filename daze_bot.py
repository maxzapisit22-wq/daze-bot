...
Commands: !clan  !stat  !rank  !rank2  !กิจ  !alert  !help
"""
...
LEADER_ROLE   = os.getenv("LEADER_ROLE", "Leader")
...
def _refresh_cache():
    ...
    codes_m  = ws_list.col_values(2)[4:]
    names_m  = ws_list.col_values(3)[4:]
    ...
    for i, name in enumerate(names_m):
        if name.strip():
            score = 0.0
            if i < len(scores):
                try: score = float(str(scores[i]).replace(",", ""))
    ...
    names_a = ws_act.col_values(2)[5:]
    att_raw = ws_act.col_values(41)[5:]
    activity = []
    for i, name in enumerate(names_a):
        if name.strip():
            att = 0
            if i < len(att_raw):
                try: att = int(str(att_raw[i]).replace(",", ""))
                except ValueError: att = 0
...
def find_member(name: str):
    n = name.strip().lower()
    m = next((x for x in get_members() if n == x["code"].lower() or n in x["name"].lower()), None)
...
async def help_cmd(ctx):
    embed = discord.Embed(title="🤖 DAZE Bot — คำสั่งทั้งหมด", color=0x5865F2)
    embed.add_field(name="`!clan`",         value="สรุปภาพรวม Clan", inline=False)
    embed.add_field(name="`!stat <ชื่อ>`", value="ดู stat รายคน", inline=False)
    embed.add_field(name="`!rank`",         value="🏆 อันดับ 1–25", inline=False)
    embed.add_field(name="`!rank2`",        value="🏆 อันดับ 26–50", inline=False)
    embed.add_field(name="`!กิจ [ชื่อ]`",  value="Top 10 กิจกรรม หรือดูรายคน", inline=False)
    embed.add_field(name="`!alert` 🔒", value="รายชื่อกิจน้อย (เฉพาะ Leader)", inline=False)
...
async def clan_cmd(ctx):
    ...
    embed.add_field(name="✅ Active (≥25)", value=f"**{active_25}** คน", inline=True)
    embed.add_field(name="⚠️ ปานกลาง",  value=f"**{active_20-active_25}** คน", inline=True)
    embed.add_field(name="🔴 กิจน้อย",   value=f"**{low_att}** คน", inline=True)
    if top_member:
...
async def stat_cmd(ctx, *, name: str = None):
    if not name:
        await ctx.send("❌ ระบุชื่อด้วยนะครับ")
        return
    ...
    embed = discord.Embed(title=f"👤 {member['name']}{code_str}",
    ...
    embed.add_field(name=f"{att_emoji(att)} กิจกรรม", value=f"**{att}/36** — {att_label(att)}", inline=True)
    embed.set_footer(text=updated_footer())
    await ctx.send(embed=embed)

async def send_rank_slice(ctx, start: int, end: int):
    members  = get_members()
    sorted_m = sorted(members, key=lambda x: x["score"], reverse=True)
    medals   = ["🥇","🥈","🥉"]
    slice_m  = sorted_m[start-1:end]
    if not slice_m:
        await ctx.send(f"❌ ไม่มีข้อมูลอันดับ {start}–{end}")
        return
    lines = []
    ...
    actual_end = start + len(slice_m) - 1
    embed = discord.Embed(title=f"🏆 Ranking อันดับ {start}–{actual_end} — DAZE",
...
async def rank_cmd(ctx):
    await send_rank_slice(ctx, 1, 25)

@bot.command(name="rank2")
async def rank2_cmd(ctx):
    await send_rank_slice(ctx, 26, 50)
...
async def activity_cmd(ctx, *, name: str = None):
    if name:
        ...
        if not act:
            ...
            return
        att = act["attendance"]
        embed = discord.Embed(title=f"📅 กิจกรรม — {act['name']}",
            color=0x57F287 if att>=25 else (0xFEE75C if att>=20 else 0xED4245))
        ...
        lines = [f"`{i+1:>2}.` {att_emoji(a['attendance'])} **{a['name']}** — {a['attendance']}/36" for i,a in enumerate(activity)]
        embed = discord.Embed(title="📅 Top 10 กิจกรรม — DAZE", description="\n".join(lines), color=0x57F287)
        embed.set_footer(text=updated_footer())
        await ctx.send(embed=embed)
...
async def alert_cmd(ctx):
    activity = get_activity()
    low = sorted([a for a in activity if a["attendance"] < 20], key=lambda x: x["attendance"])
    if not low:
        await ctx.send(embed=discord.Embed(title="✅ ไม่มีสมาชิกกิจน้อยกว่า 20 ครั้ง!", color=0x57F287))
        return
    lines = [f"🔴 **{a['name']}** — {a['attendance']}/36 ครั้ง" for a in low]
    embed = discord.Embed(title=f"⚠️ Alert — {len(low)} คน มีกิจน้อยกว่า 20 ครั้ง",
